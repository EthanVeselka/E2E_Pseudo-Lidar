...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        # .cuda should automatically transform to tensor, Variable is deprecated
        # print(refimg_fea.size().item())
        cost = (
            torch.Tensor(
                refimg_fea.size()[0],
                refimg_fea.size()[1] * 2,
        ...
        if self.training:
            cost1 = F.upsample(
                cost1, [self.maxdisp, left.size()[2], left.size()[3]], mode="trilinear"
            )
            cost2 = F.upsample(
                cost2, [self.maxdisp, left.size()[2], left.size()[3]], mode="trilinear"
            )
        ...
        cost3 = F.upsample(
            cost3, [self.maxdisp, left.size()[2], left.size()[3]], mode="trilinear"
        )
