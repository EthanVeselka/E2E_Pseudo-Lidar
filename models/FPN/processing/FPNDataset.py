...
import fpn_util as utils

BASE_DIR = "../../../carla_data/output"
ROOT_DIR = "../../.."
...
class FPNDataset(object):
    """Load and parse object data into a usable format."""

    def __init__(self, root_dir, split="training", idx_filename="train.csv"):
        """root_dir contains training and testing folders"""
        self.root_dir = root_dir
        self.split = split
        self.idx_filename = idx_filename

        frames = [frame.rstrip() for frame in open(idx_filename)]
        self.data = [os.path.join(self.root_dir, frame) for frame in frames]
...
def show_lidar_with_boxes(
    pc_velo, objects, calib, img_fov=False, img_width=None, img_height=None
):
...
def dataset_viz():
    dataset = FPNDataset(os.path.join(ROOT_DIR, "dataset/KITTI/object"))
