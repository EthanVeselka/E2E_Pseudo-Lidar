...
parser.add_argument("--log_dir", default="../train/log_v2", help="Log dir [default: log_v2]")
parser.add_argument(
...
parser.add_argument(
    "--no_intensity", action="store_true", help="Only use XYZ for training"
)
parser.add_argument(
    "--restore_model_path",
    default=None,
    ...
    with tf.Graph().as_default():
        with tf.device("/gpu:" + str(GPU_INDEX)):
            ...
            end_points = MODEL.get_model(
                pointclouds_pl, one_hot_vec_pl, is_training_pl, bn_decay=bn_decay
            )
            loss = MODEL.get_loss(
                labels_pl,
                centers_pl,
            ...
            correct = tf.equal(
                tf.argmax(end_points["mask_logits"], 2), tf.to_int64(labels_pl)
            )
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        (
            ...
            TRAIN_DATASET, train_idxs, start_idx, end_idx, NUM_POINT, NUM_CHANNEL
        )
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            log_string(
                "segmentation accuracy: %f" % (total_correct / float(total_seen))
            )
            log_string(
                "box IoU (ground/3D): %f / %f"
                % (
            ...
            log_string(
                "box estimation accuracy (IoU=0.7): %f"
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        (
            ...
            TEST_DATASET, test_idxs, start_idx, end_idx, NUM_POINT, NUM_CHANNEL
        )
    ...
    log_string(
        "eval segmentation avg class acc: %f"
        % (
            np.mean(
    ...
    log_string(
        "eval box estimation accuracy (IoU=0.7): %f"
