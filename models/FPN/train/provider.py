...
class FrustumDataset(object):
    ...
    def __init__(
        ...
        if from_rgb_detection:
            ...
            with open(overwritten_data_path, "rb") as fp:
                ...
                # frustum_angle is clockwise angle from positive x-axis
                self.frustum_angle_list = pickle.load(fp)
                self.path_list = pickle.load(fp)
                self.obj_idx_list = pickle.load(fp)
    ...
    def __getitem__(self, index):
        ...
        size_class, size_residual = size2class(
            self.size_list[index], self.type_list[index]
        )
    ...
    def get_box3d_center(self, index):
        ...
        box3d_center = (
            self.box3d_list[index][0, :] + self.box3d_list[index][6, :]
        ) / 2.0
        return box3d_center
    ...
    def get_center_view_box3d_center(self, index):
        ...
        return rotate_pc_along_y(
            np.expand_dims(box3d_center, 0), self.get_center_view_rot_angle(index)
        ).squeeze()
    ...
    def get_center_view_box3d(self, index):
        ...
        return rotate_pc_along_y(
            box3d_center_view, self.get_center_view_rot_angle(index)
        )
...
def compute_box3d_iou(
    ...
    for i in range(batch_size):
        heading_angle = class2angle(
            heading_class[i], heading_residual[i], NUM_HEADING_BIN
        )
        box_size = class2size(size_class[i], size_residual[i])
        corners_3d = get_3d_box(box_size, heading_angle, center_pred[i])

        heading_angle_label = class2angle(
            heading_class_label[i], heading_residual_label[i], NUM_HEADING_BIN
        )
        box_size_label = class2size(size_class_label[i], size_residual_label[i])
        corners_3d_label = get_3d_box(
            box_size_label, heading_angle_label, center_label[i]
        )

        iou_3d, iou_2d = box3d_iou(corners_3d, corners_3d_label)
        iou3d_list.append(iou_3d)
        iou2d_list.append(iou_2d)
    return np.array(iou2d_list, dtype=np.float32), np.array(
        iou3d_list, dtype=np.float32
    )
...
if __name__ == "__main__":
    ...
    dataset = FrustumDataset(
        1024, split="val", rotate_to_center=True, random_flip=True, random_shift=True
    )
    for i in range(len(dataset)):
        ...
        print(
            (
                ...
                data[6],
                "real_size:",
                g_type_mean_size[g_class2type[data[5]]] + data[6],
            )
        )
        ...
        box3d_from_label = get_3d_box(
            class2size(data[5], data[6]), class2angle(data[3], data[4], 12), data[2]
        )
        ...
        mlab.points3d(
            0, 0, 0, color=(1, 1, 1), mode="sphere", scale_factor=0.2, figure=fig
        )
